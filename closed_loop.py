...
# ---------------------------------------------------------------------------
# Closed-loop parameter optimisation
# ---------------------------------------------------------------------------

def perturb_params(p: float, r: float, t: int, a: float,
                   step_p: float = 0.01, step_r: float = 0.5,
                   step_a: float = 0.05) -> tuple:
    """
    Apply a small random perturbation to each percolation parameter.

    p and a are clipped to (0, 1); r is clipped to [1, inf); t stays fixed
    as an integer (the caller may vary it separately if desired).

    Returns (p_new, r_new, t_new, a_new).
    """
    p_new = float(np.clip(p + np.random.uniform(-step_p, step_p), 1e-3, 0.999))
    r_new = float(max(1.0, r + np.random.uniform(-step_r, step_r)))
    a_new = float(np.clip(a + np.random.uniform(-step_a, step_a), 0.0, 0.999))
    return p_new, r_new, t, a_new


def closed_loop_optimise(
    autostereogram: np.ndarray,
    groundtruth: np.ndarray,
    experiment_folder: str,
    # Starting percolation parameters
    init_p: float = 0.18,
    init_r: float = 2.0,
    init_t: int   = 3,
    init_a: float = 0.1,
    ...
    """
    Closed-loop optimiser: runs the forward model, measures depth-reconstruction
    correlation against the ground truth, and uses hill-climbing with random
    restarts to update the percolation parameters (p, r, t, a).

    Algorithm (per iteration):
      1. Run the full forward path with current parameters.
      2. Record the correlation score.
      3. Perturb parameters randomly within small step sizes.
      4. If the new score is better, keep the perturbation (accept).
         Otherwise revert (reject) and increment the patience counter.
      5. After `patience` consecutive rejections, reset to the best parameters
         found so far and apply a larger perturbation (restart).

    Returns a list of dicts, one per iteration, each containing the
    parameters used and the correlation score achieved.
    """
    p, r, t, a  = init_p, init_r, init_t, init_a
    best_corr   = -1.0
    best_params = (p, r, t, a)
    no_improve  = 0
    history     = []
    ...
    print("=" * 60)
    print("CLOSED-LOOP OPTIMISATION")
    print(f"  max_iterations={max_iterations}  patience={patience}")
    print(f"  starting params: p={p:.4f}  r={r:.2f}  t={t}  a={a:.3f}")
    print("=" * 60)

    for iteration in range(1, max_iterations + 1):
        print(f"\n--- Iteration {iteration}/{max_iterations} ---")
        print(f"  params: p={p:.4f}  r={r:.2f}  t={t}  a={a:.3f}")
        ...
        result = run_experiment(
            ...
            patch_size=patch_size,
            lgn_width=lgn_width,
            lgn_p=p, lgn_r=r, lgn_t=t, lgn_a=a,
            autostereogram=autostereogram,
            asg_patch_size=asg_patch_size,
            groundtruth=groundtruth,
            experiment_folder=experiment_folder,
        )

        corr = result["corr"]
        history.append({**result, "iteration": iteration})
        print(f"  correlation: {corr:.4f}  (best so far: {best_corr:.4f})")
        ...
        if corr > best_corr:
            ...
            no_improve += 1
            print(f"  -> Rejected ({no_improve}/{patience} patience)")

        if no_improve >= patience:
            print("  ** Patience exhausted - restarting from best parameters **")
            p, r, t, a = best_params
            no_improve = 0
            # Larger perturbation on restart to escape local optima
            p, r, t, a = perturb_params(p, r, t, a,
                                         step_p=step_p * 3,
                                         step_r=step_r * 3,
                                         step_a=step_a * 3)
        else:
    ...
    bp = best_params
    print(f"  best parameters  : p={bp[0]:.4f}  r={bp[1]:.2f}  t={bp[2]}  a={bp[3]:.3f}")
    print("=" * 60)

    history_path = f"{experiment_folder}/json/optimisation_history.json"
...
def plot_optimisation_history(history: list, save_path: str = None) -> None:
    """
    Plot correlation score vs iteration from a closed-loop optimisation run.

    Marks the best-scoring iteration with a red star and optionally saves to disk.
    """
    iterations = [h["iteration"] for h in history]
    scores     = [h["corr"]      for h in history]
    best_idx   = int(np.argmax(scores))

    fig, ax = plt.subplots(figsize=(6, 3.5))
    ax.plot(iterations, scores, color="#2c7bb6", linewidth=1.2, zorder=2)
    ax.scatter(iterations, scores, s=20, color="#2c7bb6", zorder=3)
    ax.scatter(iterations[best_idx], scores[best_idx],
               s=120, marker="*", color="#d7191c", zorder=4,
               label=f"Best: {scores[best_idx]:.4f} (iter {iterations[best_idx]})")

    ax.set_xlabel("Iteration", fontsize=9)
    ax.set_ylabel("Depth correlation", fontsize=9)
    ax.set_title("Closed-loop optimisation: correlation vs iteration",
...
def main():
    ...
    history = closed_loop_optimise(
        autostereogram=autostereogram,
        groundtruth=groundtruth,
        experiment_folder=parent_path,
        init_p=0.18, init_r=2.0, init_t=3, init_a=0.1,
        num_filters=100, num_components=20,
        num_patches=10000, patch_size=9,
        lgn_width=256, asg_patch_size=70,
        max_iterations=30,
        patience=5,
    )
    plot_optimisation_history(
        history,
        save_path=f"{parent_path}/images/optimisation_history.png",
    )
