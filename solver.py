...
def magiceye_solver(x):
    ...
    for i in range(c):
        ...
        filt_1 = scp.prewitt(shifted)
        filt_2 = scp.uniform_filter(filt_1, size=(5, 5))
        if ski_filter:
...
image = plt.imread(r"C:\vscode\innate-binocular-vision\innate-binocular-vision\data\data.png") #load magiceye image

solution = magiceye_solver(image) #solve it
print(solution.shape)
plt.imshow(solution, cmap = plt.cm.gray) 
plt.show() 